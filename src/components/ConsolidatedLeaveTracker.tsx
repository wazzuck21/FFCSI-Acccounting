...
export const ConsolidatedLeaveTracker: React.FC<ConsolidatedLeaveTrackerProps> = () => {
  ...
  const handleFileLeaveSubmit = (e: React.FormEvent) => {
    ...
    addAuditLog(
      'LEAVE_REQUEST',
      `Filed leave for ${emp.fullName}: ${leaveForm.leaveType} (${leaveForm.totalDays} day/s from ${leaveForm.startDate} to ${leaveForm.endDate}). Status: ${isSuperAdmin ? 'Approved' : 'Pending'}.`,
      currentUser?.id,
